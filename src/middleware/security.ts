...
const securityMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try{
        ...
        let message: string;
         switch(role) {
            case 'admin' :
                limit = 20 ;// 2 requests per interval
                message = "Admin rate limit exceeded. Please try again later(20 per minute).";
                break;

                case 'teacher':
                case 'student':
                    limit = 10; // 1 request per interval
                    message = "User rate limit exceeded. Please try again later(10 per minute).";
                    break;
                    
                default:
                    limit = 5; // default limit for unknown roles
                    message = "Guest rate limit exceeded. Please signup for higher limits).";
         }
         ...
         if (decision.isDenied() && decision.reason.isRateLimit()) {
            return res.status(403).json({ error:"Too many requests. Please try again later.",message });
         }
         next();
    }catch(e) {
